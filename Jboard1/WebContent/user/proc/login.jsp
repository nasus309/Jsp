...
<%
	...
	// 3단계
	// PreparedStatement 는 미리 sql문을 지정해 놓는다. 
	// ?는 자료형 신경쓰지 않아도 된다!
	String sql = "SELECT * FROM `JBOARD_USER` WHERE `uid`=? AND `pass`=SHA2(?, 224);";
	PreparedStatement psmt = conn.prepareStatement(sql);
	psmt.setString(1, uid);  // psmt의 1번 째 ?
	psmt.setString(2, pass); // psmt의 2번 째 ?
	...
	if(rs.next()){
		...
		session.setAttribute("suser", user); // 로그인에 성공하고 나면 session에 사용자의 정보를 저장함.
